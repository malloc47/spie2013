...
clicks) and time.  The results of this comparative experiment are
shown in \fig{ex}.  Note that the first slice of the proposed method
is used as an initialization, so it requires significantly more effort
and time to segment compared with the remaining slices.
\begin{figure}[htbp]
\centering
...
effort ($< 100$ clicks in most cases).  The intelligent scissors
method, without the benefit of an initialization, requires
